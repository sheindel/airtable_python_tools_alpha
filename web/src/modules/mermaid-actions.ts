/**
 * Mermaid Actions Module
 * Handles actions related to Mermaid diagrams, including opening in Mermaid Live,
 * copying to clipboard, downloading, and fullscreen toggle.
 */

import { toast } from "./toast.js";

/**
 * Decode Mermaid state (debugging function)
 */
function decodeMermaidState(): string {
    ...
    const compressedGraph = (window as any).Base64.toUint8Array(x, true);
    const graphDefinition = (window as any).pako.inflate(compressedGraph, { to: "string" });
    console.log("Graph definition:", graphDefinition);
    return graphDefinition;
}

/**
 * Open the dependency diagram in Mermaid Live Editor
 */
export function openInMermaidLive(): void {
    decodeMermaidState();
...
/**
 * Copy the dependency diagram Mermaid text to clipboard
 */
export function copyMermaidText(): void {
    const graphDefinition = localStorage.getItem("lastGraphDefinition");
...
/**
 * Toggle fullscreen mode for the Mermaid container
 */
...
/**
 * Download the Mermaid diagram as a text file
 */
export function downloadMermaidText(): void {
    const graphDefinition = localStorage.getItem("lastGraphDefinition");
...
/**
 * Download the Mermaid diagram as SVG
 */
export function downloadMermaidSVG(): void {
